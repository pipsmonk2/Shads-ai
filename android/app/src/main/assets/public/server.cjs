...
async function startServer() {
  ...
  app.get(
    ...
    (req, res) => {
      const candidates = [
        import_path.default.resolve(process.cwd(), "APK_DOWNLOAD", "ShadsAI_v1.0.apk"),
        import_path.default.resolve(process.cwd(), "APK_DOWNLOAD", "app-release.apk"),
        import_path.default.resolve(process.cwd(), "APK_DOWNLOAD", "app-debug.apk"),
        import_path.default.resolve(process.cwd(), "apk", "ShadsAI_v1.0.apk"),
        import_path.default.resolve(process.cwd(), ".build-outputs", "ShadsAI_v1.0.apk"),
        import_path.default.resolve(process.cwd(), ".build-outputs", "app-debug.apk"),
        ...
        import_path.default.resolve(process.cwd(), "public", "shads_ai.apk"),
        import_path.default.resolve(process.cwd(), "ShadsAI_v1.0.apk")
      ];
  ...
  const CACHE_DURATION_MS = 15 * 60 * 1e3;
  const FAIL_CACHE_DURATION_MS = 2 * 60 * 1e3;
  ...
  async function callGeminiWithRetryAndFallback(ai, generateParams, candidateModels = ["gemini-3.1-flash-lite", "gemini-flash-latest", "gemini-3.7-flash"]) {
    let lastError = null;
...
Also include "preNewsStrategy" (15m before news) and "postNewsStrategy" (15m after news). Keep the list to 3-5 high-impact events.`;
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                ...
                // "BULLISH_USD" | "BEARISH_USD" | "MIXED" | "NEUTRAL"
                headlineSummary: { type: "STRING" },
                // A concise, high-level summary of the overall market sentiment
                events: {
                  type: "ARRAY",
                  items: {
                    type: "OBJECT",
                    properties: {
                      title: { type: "STRING" },
                      // E.g., "US Non-Farm Payrolls (NFP) & Unemployment"
                      impact: { type: "STRING" },
                      // "HIGH" | "MEDIUM" | "LOW"
                      ...
                      // "BULLISH" | "BEARISH" | "HIGH_VOLATILITY" | "NEUTRAL"
                      directionalReasoning: { type: "STRING" },
                      timeUntil: { type: "STRING" },
                      // E.g. "Today 13:30 GMT" or "In 45 Mins"
                      scheduledTimestamp: { type: "INTEGER" },
                      // Epoch timestamp in milliseconds for live countdown clock
                      expectedPipVolatility: { type: "STRING" },
                      // E.g. "90 - 140 Pips"
                      affectedPairs: {
                        type: "ARRAY",
                        items: { type: "STRING" }
                      ...
                      pairRecommendations: {
                        ...
                        items: {
                          ...
                          properties: {
                            ...
                            // "BUY NOW" | "SELL NOW" | "BUY LIMIT" | "SELL LIMIT" | "BUY STOP" | "SELL STOP"
  ...
  function getFallbackScan(pair, timeframe) {
    ...
    const momentumStrategy = signal === "BUY" ? `High-velocity bullish momentum expansion surging with strong volume candles on ${finalTimeframe}, confirming buyer dominance.` : `Strong bearish momentum thrust breaking lower with expanding candlestick bodies, signaling sustained seller pressure.`;
    const reversalStrategy = signal === "BUY" ? `Bullish reversal setup confirmed following a clean liquidity sweep of swing low and immediate rejection pinbar at ${p(basePrice * 0.998)}.` : `Bearish reversal setup validated after liquidity raid of equal highs and rejection candle forming at ${p(basePrice * 1.002)}.`;
    const breakAndRetestStrategy = signal === "BUY" ? `Break & Retest strategy active: broken resistance level at ${p(basePrice * 1)} cleanly retested as new institutional support.` : `Break & Retest strategy active: broken support floor at ${p(basePrice * 1)} retested from below as new supply ceiling.`;
    const marketStructureAnalysis = {
      marketStructureShift: signal === "BUY" ? `Bullish Market Structure Shift (MSS) confirmed on ${finalTimeframe} with close above recent lower high.` : `Bearish Market Structure Shift (MSS) confirmed on ${finalTimeframe} with close below recent higher low.`,
      breakOfStructure: signal === "BUY" ? `Decisive Break of Structure (BOS) past ${p(basePrice * 1.002)} confirming macro uptrend expansion.` : `Decisive Break of Structure (BOS) past ${p(basePrice * 0.998)} confirming macro downtrend expansion.`,
      changeOfCharacter: signal === "BUY" ? `Change of Character (CHoCH) detected as price holds higher low at ${p(basePrice * 0.999)}.` : `Change of Character (CHoCH) validated following failure at ${p(basePrice * 1.0015)}.`,
      breakoutStrategy: signal === "BUY" ? `High probability range breakout above resistance at ${p(basePrice * 1.001)} backed by volume spike.` : `High probability breakdown below key support floor at ${p(basePrice * 0.999)} with heavy volume.`,
      pullbackStrategy: signal === "BUY" ? `Controlled pullback into 0.618 OTE discount zone at ${p(basePrice * 0.9995)} presenting prime long entry.` : `Controlled pullback into premium pricing zone at ${p(basePrice * 1.0005)} presenting prime short entry.`,
      sessionKillZones: signal === "BUY" ? `London Kill Zone (08:00 UTC) range sweep established daily low; NY session continuation active.` : `NY Open Kill Zone (13:30 UTC) liquidity raid at session high sets up daily distribution reversal.`,
      momentumStrategy,
      reversalStrategy,
    ...
    const harmonicWaveAnalysis = {
      fibonacciRetracement: signal === "BUY" ? `Entry aligns with 61.8% / 78.6% Optimal Trade Entry (OTE) golden Fib zone at ${p(basePrice * 0.9995)}.` : `Entry aligns with 61.8% / 78.6% OTE premium Fib zone at ${p(basePrice * 1.0005)}.`,
      elliottWaves: signal === "BUY" ? `Wave 3 Impulse Leg in progress; Wave 2 correction completed at ${p(basePrice * 0.999)}.` : `Wave 5 terminal impulse complete; Wave C corrective distribution phase accelerating downward.`,
      harmonicPatterns: signal === "BUY" ? `Bullish Bullish Gartley / Bat Pattern Potential Reversal Zone (PRZ) completed at ${p(basePrice * 0.9992)}.` : `Bearish Butterfly Pattern PRZ completed at ${p(basePrice * 1.0018)} with 1.272 extension D-leg.`,
      wyckoffMethod: signal === "BUY" ? `Wyckoff Accumulation Phase C: Spring test completed below support, initiating Sign of Strength (SOS).` : `Wyckoff Distribution Phase C: Upthrust After Distribution (UTAD) completed, initiating Sign of Weakness (SOW).`
    };
    const volumeSessionAnalysis = {
      volumeProfile: signal === "BUY" ? `Point of Control (POC) established at ${p(basePrice * 0.9992)}; price bouncing cleanly off Value Area Low (VAL).` : `Point of Control (POC) established at ${p(basePrice * 1.0008)}; price rejecting off Value Area High (VAH).`,
      vwapAnalysis: signal === "BUY" ? `Price holding firmly above Session VWAP and +1 Std Dev band at ${p(basePrice * 0.9996)}.` : `Price trading below Session VWAP and -1 Std Dev band at ${p(basePrice * 1.0004)}.`,
      sessionBreakouts: signal === "BUY" ? `Asian Session Range high swept; London Open expansion breaking out upward.` : `Asian Session Range low swept; London Open expansion breaking out downward.`
    };
    const strategiesMatrix = [
      { id: "mom", name: "Momentum Trading Strategy", category: "structure", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 94, details: momentumStrategy },
      { id: "bo", name: "Breakout Strategy", category: "structure", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 91, details: marketStructureAnalysis.breakoutStrategy },
      { id: "rev", name: "Reversal Strategy", category: "smc", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 93, details: reversalStrategy },
      { id: "br", name: "Break and Retest Strategy", category: "structure", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 92, details: breakAndRetestStrategy },
      { id: "sr", name: "Support & Resistance", category: "technicals", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 88, details: technicalAnalysis.supportResistance },
      { id: "sd", name: "Supply & Demand", category: "technicals", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 91, details: technicalAnalysis.supplyDemand },
      { id: "ob", name: "Order Block", category: "smc", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 94, details: smcAnalysis.orderBlocks },
      { id: "fib", name: "Fibonacci Retracement", category: "harmonic", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 89, details: harmonicWaveAnalysis.fibonacciRetracement },
      { id: "ew", name: "Elliott Waves", category: "harmonic", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 85, details: harmonicWaveAnalysis.elliottWaves },
      { id: "harmonic", name: "Harmonic Patterns", category: "harmonic", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 86, details: harmonicWaveAnalysis.harmonicPatterns },
      { id: "wyckoff", name: "Wyckoff Method", category: "harmonic", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 90, details: harmonicWaveAnalysis.wyckoffMethod },
      { id: "vp", name: "Volume Profile", category: "volume", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 92, details: volumeSessionAnalysis.volumeProfile },
      { id: "vwap", name: "VWAP", category: "volume", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 87, details: volumeSessionAnalysis.vwapAnalysis },
      { id: "tl", name: "Trend Lines", category: "technicals", status: "BULLISH", confidence: 83, details: technicalAnalysis.trendlines },
      { id: "cp", name: "Chart Patterns", category: "technicals", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 89, details: technicalAnalysis.chartPatterns },
      { id: "candle", name: "Candlestick Patterns", category: "technicals", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 85, details: technicalAnalysis.candlestickPattern },
      { id: "pb", name: "Pullback Strategies", category: "structure", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 90, details: marketStructureAnalysis.pullbackStrategy },
      { id: "sbo", name: "Session Breakouts", category: "volume", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 86, details: volumeSessionAnalysis.sessionBreakouts },
      { id: "kz", name: "Range + Kill Zones", category: "structure", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 93, details: marketStructureAnalysis.sessionKillZones },
      { id: "ls", name: "Liquidity Sweeps", category: "smc", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 95, details: smcAnalysis.liquiditySweeps },
      { id: "fvg", name: "Fair Value Gaps (FVG)", category: "smc", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 91, details: smcAnalysis.marketImbalance },
      { id: "rb", name: "Rejection Blocks", category: "smc", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 87, details: smcAnalysis.rejectionBlocks },
      { id: "mb", name: "Mitigation Blocks", category: "smc", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 89, details: smcAnalysis.mitigationBlocks },
      { id: "disp", name: "Displacement", category: "smc", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 94, details: smcAnalysis.displacement },
      { id: "mss", name: "Market Structure Shift (MSS)", category: "structure", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 93, details: marketStructureAnalysis.marketStructureShift },
      { id: "bos", name: "Break of Structure (BOS)", category: "structure", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 91, details: marketStructureAnalysis.breakOfStructure },
      { id: "choch", name: "Change of Character (CHoCH)", category: "structure", status: signal === "BUY" ? "BULLISH" : "BEARISH", confidence: 88, details: marketStructureAnalysis.changeOfCharacter }
    ];
    ...
    let orderType = signal === "BUY" ? isMarketExecution ? "BUY NOW" : "BUY LIMIT" : isMarketExecution ? "SELL NOW" : "SELL LIMIT";
    let orderTypeCategory = isMarketExecution ? "MARKET" : "LIMIT";
    ...
    const voiceSummary = signal === "BUY" ? `${cleanPairNameForSpeech} indicates a ${orderType} setup on the ${finalTimeframe} chart. All 24 trading engines including market structure shift, order block, and Fibonacci retracement validate the bullish signal. Target TP1 through TP6 up to ${takeProfit6} with stop loss at ${stopLoss}.` : `${cleanPairNameForSpeech} indicates a ${orderType} setup on the ${finalTimeframe} chart. Bearish order flow, VWAP distribution, and liquidity sweeps confirm the short signal. Target TP1 through TP6 down to ${takeProfit6} with stop loss at ${stopLoss}.`;
    return {
      signal,
      orderType,
      orderTypeCategory,
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                signal: { type: "STRING" },
                orderType: { type: "STRING" },
                // "BUY NOW" | "SELL NOW" | "BUY LIMIT" | "SELL LIMIT" | "BUY STOP" | "SELL STOP" | "WAIT"
                orderTypeCategory: { type: "STRING" },
                // "MARKET" | "LIMIT" | "STOP" | "WAIT"
